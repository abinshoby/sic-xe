...
3)TO EXECUTE FOLLOW THE FOLLOWING INSTRUCTIONS,
	I)COMPILE USING THE COMMAND g++ main.cpp -o <obj name>
	II)EXECUTE USING THE COMMAND ./<obj name> <name of the sic/xe source file to be assembled>
4)THE INTERMEDIATE FILE IS AVAILABLE AS  "name of the sic/xe source file.int"
5)THE OBJECT FILE IS AVAILABLE AS  "name of the sic/xe source file.obj"
6)THE 
...
int pass1(int lineno,string label,string opcode,string operand,char argv[]){
	...
	//string str(argv);
	//string ff=str+"_intermediate.txt";
	char ff2[20];
	int i;
	strcpy(ff2,argv);
	...
	if(label.compare("")!=0){
		...
		if(stat==1)
		{error=1;cout<<"Error at line:"<<lineno<<"undefined symbol "<<label<<endl;}
		else
	...
	{error=2; cout<<"Error at line:"<<lineno<<"Invalid operation "<<opcode<<endl;}
	...
	pgml<<res_pgmlength<<"\t"<<PGM_NAME<<endl;
	pgml.close();
...
int main(int argc,char *argv[]){
	...
	{       
		...
		{	if(line.compare("")==0){
			...
			lineno++;
			cout<<lineno<<"::Assembling the instruction:"<<line<<endl;
			//cout<<"line no"<<lineno;
			//cout<<line<<endl;
			if(line.at(0)=='.')
				continue;
	...
	fstream symtab;
	symtab.open("symbol_table.txt",ios::out);
	for(int i=0;i<symtabsz;i++)
		symtab<<SYMTAB[i][0]<<"\t"<<SYMTAB[i][1]<<endl;
	symtab.close();
	char para[25];
	i=0;
	para[i++]='.';
